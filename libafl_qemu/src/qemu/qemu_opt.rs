...
#[builder(build_method(vis = "", name = build_config), builder_method(vis = "pub(crate)"))]
...
impl QemuConfig {
    fn build(self) -> Result<Qemu, QemuInitError> {
...
mod test {
    ...
    fn default_fmt_is_empty() {
        let opt = QemuConfig::builder().build_config();
    ...
    #[test]
    fn accelerator_fmt() {
        let opt = Qemu::builder().accelerator(Accelerator::kvm).build_config();
